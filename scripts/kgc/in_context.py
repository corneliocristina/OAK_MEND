...
from okgc.in_context.in_context import InContextKGC
...
parser = argparse.ArgumentParser(
    prog="Experiment In-Context KGC Launcher",
...
parser.add_argument(
    ...
    type=int,
    default=20,
    help="The maximum number of predicates to insert in the LLM context",
)
parser.add_argument(
...
def run_experiment(entry_idx: int):
    ...
    try:
        for sub_text_id, text in enumerate(entry.texts):
            ...
            kg, gen_usage = inctx_method.generate_from_text(text, text_id=text_id)
            kgs.append(kg)
            for k, u in gen_usage.items():
                usage_info[k] += u
        kg = merge_graphs(kgs)
        kg, dedup_usage = inctx_method.deduplicate_entities(
...
if __name__ == "__main__":
    ...
    outputs_path = os.path.join(
        "results",
        "kgc-inctx" if not args.out_alias else f"kgc-inctx-{args.out_alias}",
        args.dataset,
    ...
    # Initialize the In-Context KGC method object
    inctx_method = InContextKGC(
        client,
        sent_embed,
