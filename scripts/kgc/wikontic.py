...
from okgc.wikontic.wikontic import Wikontic
from scripts.utils import ranges_to_indices

parser = argparse.ArgumentParser(
    prog="Experiment Wikontic Launcher",
...
def run_experiment(entry_idx: int):
    ...
    try:
        wikontic.drop_mongo_db_rows(entry_idx)
        for sub_text_id, text in enumerate(entry.texts):
            text_id = (entry_idx << 16) + sub_text_id
            kg, gen_usage = wikontic.generate_from_text(text, text_id=text_id)
            kgs.append(kg)
            for k, u in gen_usage.items():
                usage_info[k] += u
        kg = merge_graphs(kgs)
        kg.entity_aliases = wikontic.retrieve_entity_aliases(entry_idx)
    except Exception:
...
if __name__ == "__main__":
    ...
    # Initialize the wikontic object
    wikontic = Wikontic(
        args.llm,
        args.llm_api_base,
        args.api_key,
    ...
    outputs_path = os.path.join(
        "results",
        "kgc-wikontic" if not args.out_alias else f"kgc-wikontic-{args.out_alias}",
        args.dataset,
        format_llm_name(wikontic.engine.extractor.model),
        build_results_path(
            seed=args.seed,
        ),
    )
