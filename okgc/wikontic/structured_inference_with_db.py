###############################################################################
#
# okgc/wikontic/structured_inference_with_db.py
#
# Code originally from src/wikontic/utils/structured_inference_with_db.py in
...
from ..utils.usage import UsageInfo
...
class StructuredInferenceWithDB(BaseInferenceWithDB):
    ...
    def extract_triplets_with_ontology_filtering(
        ...
        self.extractor.reset_tokens()
        usage_info["extract_triples"] = UsageInfo(
            num_prompt_tokens, num_completion_tokens
        )

        if isinstance(extracted_triplets, str):
            extracted_triplets = json_repair.loads(extracted_triplets)
        ...
        for triplet in extracted_triplets["triplets"]:
            ...
            try:
                ...
                self.extractor.reset_tokens()
                usage_info["refine_entity_types"] += UsageInfo(
                    num_prompt_tokens, num_completion_tokens
                )
                ...
                self.extractor.reset_tokens()
                usage_info["refine_predicates"] += UsageInfo(
                    num_prompt_tokens, num_completion_tokens
                )
                ...
                self.extractor.reset_tokens()
                usage_info["refine_entity_names"] += UsageInfo(
                    num_prompt_tokens, num_completion_tokens
                )
                ...
                backbone_triplet.update(
                    {
                        "subject_type_id": refined_subject_type_id,
                        "object_type_id": refined_object_type_id,
                        "relation_id": refined_relation_id,
                    }
                )
